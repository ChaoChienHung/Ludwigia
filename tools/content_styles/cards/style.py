class CardsStyle:
    ...
    def _open_card(self) -> str:
        self._card_open = True
        return '<div class="card project-card mt-4">'

    def _close_card(self) -> str:
        self._card_open = False
        return "</div>"
    ...
    def before_heading(self, level: int) -> list[str]:
        ...
        out: list[str] = []
        if self._card_open:
            out.append(self._close_card())
        out.append(self._open_card())
        return out

    def after_document(self) -> list[str]:
        if not self._card_open:
            return []
        return [self._close_card()]

    def heading(self, level: int, inner_html: str) -> str:
        if level <= 3:
            return f'<h{level} class="mb-3">{inner_html}</h{level}>'
