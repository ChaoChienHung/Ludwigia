...
- [ ] Writing：Active recall 與測驗形式（選擇題比較「看起來輕鬆」但也可能是低保真回想）
  - [ ] 目標：閱讀時間 10–15 分鐘（超過就拆段或移到下一篇）
  - [ ] 草稿：`writing/recognition-vs-active-recall/recognition-vs-active-recall-zh-tw.md`
  - [ ] 核心比喻：辨識兔子（抓到少數特徵就能選對） vs 畫出兔子（需要完整表徵）
  - [ ] 大綱（先求可寫、再求完整）：
    - [ ] Two modes：Recognition（辨識） vs Recall（生成/重建）
    - [ ] MCQ：為什麼選擇題常常「只要知道一點點」就能猜中/選中（低保真理解的風險）
    - [ ] Free response：為什麼要能寫出來/畫出來才算真的懂（高保真回想）
    - [ ] Practice design：如何設計練習讓自己逐步從辨識 → 回想（混合題型、延遲回想、錯題再生）
  - [ ] 重點整理／觀察（寫作提示）：
    - [ ] 選擇題不是無用，但它測到的能力可能不是「完整理解」
    - [ ] 產出式練習（寫/畫/講）是更硬的 active recall
  - [ ] 下一輪重寫方向：
    - [ ] 把 `選擇題的陷阱與空白頁的考驗` 那段大幅壓縮，避免文章從「理解錯覺與大腦偷懶」滑到「題型設計 / 教學策略」
    - [ ] 明確把費曼學習法與小黃鴨除錯法定位成「同時破解兩個陷阱」的方法，而不只是後段例子
    - [ ] 補強「拆到第一性原理」的更大好處：不只是找到錨點或比擬，而是找到共享底層模式，讓新知識直接長成既有知識體系的延伸
    - [ ] 補更多服務同一主張的例子：英文單字 / 字根與語感模式、跨知識系統的共享 pattern、以及視情況評估是否保留 RL / Reverse KL 這類進階例子
  - [ ] 原始想法（完整保留）：
    - [ ] 其實我在想 有關於拆到第一性原理的優點 甚至還有更後面的 在知識量累積且有著許多條路徑延伸至共享模式之後 底層的架構與知識系統會變得愈發的穩固 以至于能夠讓大腦能更穩固的連結起現有的許多知識系統 整體而言就更不容易忘 也更容易記起更多的知識（這又涉及到我之前認為的 人的大腦腦容量有限 因此要盡可能地用information theory的方式去壓縮共同prefix 盡可能讓更多的資訊共用短prefix 這樣就能更好的表達所有的知識 因為所有的知識都是在接近最短編碼 所以才能在固定的腦容量裡塞入更多的資訊 我覺得這是直得另一篇文章的 然後另一個點就是 更多的更穩固更發達的網路不只像前面說的能夠更容易創造錨點與連結點 也更容易舉一反三更在同樣中找不同 並將這不同之處apply到其他同樣處 這本質上就是創造力的一種表現 這也是能延伸到lapolazza effect） 所以結論是我覺得為什麼對齊第一性原理滿重要的 而這部分感覺是能單獨寫另一篇文章來說的
  - [ ] 文章鏈規劃：
    - [ ] 這篇只保留和「辨識 vs 主動回想」「大腦偷懶」「兩個陷阱」直接相關的第一性原理段落
    - [ ] 另開一篇專門談「為什麼對齊第一性原理很重要」，把共享模式、共同 prefix、知識壓縮、穩固知識網路、舉一反三與創造力完整展開
    - [ ] 本文未來只需留一句過渡：至於為什麼拆到第一性原理不只是幫助當下理解，還會進一步影響記憶穩固、知識壓縮、共享模式的建立與跨領域遷移，那會是另一篇文章要完整展開的主題。
  - [ ] 交付條件：在 `writing/` 新增一篇可讀草稿（至少 4 個小節，每節至少 3 個 bullet），並確保 `search/search-index.{json,js}` 已更新並納入版本控制

- [ ] Writing：為什麼對齊第一性原理很重要
  - [ ] 目標：把「拆到第一性原理」的長期價值獨立成篇，不塞爆 `recognition-vs-active-recall`
  - [ ] 草稿：`writing/why-first-principles-matter/why-first-principles-matter-zh-tw.md`
  - [ ] 核心問題：為什麼當很多知識都沿著不同路徑收斂到共享模式後，底層知識網路會越來越穩固、越不容易忘、也越容易 recall / 遷移 / 舉一反三？
  - [ ] 原始想法（完整保留）：
    - [ ] 其實我在想 有關於拆到第一性原理的優點 甚至還有更後面的 在知識量累積且有著許多條路徑延伸至共享模式之後 底層的架構與知識系統會變得愈發的穩固 以至于能夠讓大腦能更穩固的連結起現有的許多知識系統 整體而言就更不容易忘 也更容易記起更多的知識（這又涉及到我之前認為的 人的大腦腦容量有限 因此要盡可能地用information theory的方式去壓縮共同prefix 盡可能讓更多的資訊共用短prefix 這樣就能更好的表達所有的知識 因為所有的知識都是在接近最短編碼 所以才能在固定的腦容量裡塞入更多的資訊 我覺得這是直得另一篇文章的 然後另一個點就是 更多的更穩固更發達的網路不只像前面說的能夠更容易創造錨點與連結點 也更容易舉一反三更在同樣中找不同 並將這不同之處apply到其他同樣處 這本質上就是創造力的一種表現 這也是能延伸到lapolazza effect） 所以結論是我覺得為什麼對齊第一性原理滿重要的 而這部分感覺是能單獨寫另一篇文章來說的
  - [ ] 大綱（先求可寫、再求完整）：
    - [ ] Shared modes：不同知識沿不同路徑收斂到共享模式後，如何讓底層網路越來越穩
    - [ ] Compression：用共同 prefix / information theory 的角度理解知識壓縮與腦容量限制
    - [ ] Recall and retention：為什麼共享底層模式越多，越不容易忘，也越容易記起更多知識
    - [ ] Transfer and creativity：為什麼更穩固的網路會讓人更容易舉一反三、同中找異、把不同之處再 apply 回其他同樣處
  - [ ] 交付條件：至少先整理成一篇可讀 draft，並在文中清楚區分「比擬」「共享模式」「知識系統延伸」三者的關係
