...
一個分佈的「<information concept="concept.expectation">期望值</information>」，本就是由整個樣本空間的機率與其數值綜合推演而出的「重心」。在這裡，**「非負（大於等於零）」的前提扮演了至關重要的角色**。

因為隨機變數的數值被左側的「零」給牢牢擋住（即最小邊界值為 0），我們沒有任何極端小的負數可以拿來「抵消」右側極端大值對重心的拉抬效應。這意味著，右側極端大值所能擁有的總機率（權重），是被這個重心位置與左側的零底限給嚴格夾擊、限制住的。

馬爾科夫不等式正是利用了這個特性，宣告極端大值出現的機率最多不會超過多少。道理很簡單：**如果那些極端大值出現的機率真的超過了這個界限，在缺乏負數把重心往左拉回的情況下，這些極大值所貢獻的權重，打從一開始就會把期望值往上拉高，使得這組資料的期望值根本不可能是我們目前所看到的數字。**
...
### 學術論文與專著（Academic Literature & Textbooks）

1. Taboga, Marco (2021). "Markov's inequality", *Lectures on probability theory and mathematical statistics*. Kindle Direct Publishing. Online appendix. https://www.statlect.com/fundamentals-of-probability/Markov-inequality