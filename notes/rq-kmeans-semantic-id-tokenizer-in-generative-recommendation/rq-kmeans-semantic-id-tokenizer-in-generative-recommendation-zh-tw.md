...
- 核心思維：從連續空間到離散 Token 的幾何橋樑
    - 兩步拆解難題：1. 如何將連續向量轉為帶有語意的離散 Token？ 2. 如何讓離散 Token 構成「由粗到細」的階層式序列？本章聚焦第一個問題。
...
要將連續的高維商品向量（Embeddings）轉換為大模型能理解的 Semantic ID (SID) 序列，本質上可以拆解為兩個循序漸進的核心難題：

1. **基礎問題：如何將連續向量轉換為「帶有語意」的離散 Token？**（解決連續空間 $\to$ 離散字典的映射問題）
2. **進階問題：如何讓這些離散 Token 構成「由粗到細（Coarse-to-fine）」的階層式序列？**（解決單一 Token $\to$ 階層序列的結構問題）

本章我們將聚焦於第一個基礎問題：為什麼傳統生硬的「網格分桶」會宣告失敗？而「聚類即量化」又是如何巧妙地扮演了連續空間與離散語意之間的幾何橋樑。
