...
<callout>
title: Extreme Edge Case: Empty Clusters & Single-Cluster Collapse
icon: alert-triangle
content:
While bad initialization usually causes convergence to a poor local minimum, extreme initializations can trigger the **empty cluster dilemma** ($|C_k| = 0$).

If a centroid is initialized in a remote, sparse region—or if neighboring centroids actively "steal" all surrounding data points during an update—it is possible that zero data points choose that centroid as their nearest neighbor. In the update phase ($\mu_k = \frac{1}{|C_k|} \sum x$), dividing by $|C_k| = 0$ causes a mathematical $\frac{0}{0}$ division-by-zero breakdown. 

In the most extreme scenario, if one centroid is placed right at a dense core while the remaining $K-1$ centroids start far on the outer margins, that central centroid will greedily capture **all $N$ data points**. As a result, the entire dataset collapses into a single massive cluster ($K=1$), leaving $K-1$ empty, useless centroids.

**How Engines Handle This:** Robust engines (such as scikit-learn) dynamically detect empty clusters ($|C_k|=0$) and forcibly re-assign the empty centroid to the data point $x_{\text{max}}$ currently contributing the highest single squared error in the dataset.

<rawhtml>
<div class="kmeans-widget empty-cluster-demo" id="km-ec-container">
  <div class="kmeans-header">
    <div class="kmeans-title">
      <span class="kmeans-badge alert-badge">Interactive Demo</span>
      <span>Empty Cluster & Single-Cluster Collapse ($K=3$)</span>
    </div>
    <div class="kmeans-stats">
      <span class="stat-item">Active Clusters: <strong id="km-ec-active">1 / 3</strong></span>
      <span class="stat-badge alert-stat" id="km-ec-status">Initialized</span>
    </div>
  </div>

  <div class="kmeans-canvas-wrapper">
    <canvas id="km-ec-canvas" width="600" height="240"></canvas>
  </div>

  <div class="kmeans-controls">
    <div class="controls-row config-row">
      <span class="control-label">Preset Scenario:</span>
      <div class="km-pill-group" id="km-ec-pills">
        <button type="button" class="km-pill-btn active" data-scenario="collapse">💥 Extreme Collapse</button>
        <button type="button" class="km-pill-btn" data-scenario="recover">🛠️ Re-assign <em>x</em><sub>max</sub></button>
      </div>
    </div>
    <div class="controls-row button-row">
      <button type="button" id="km-ec-step" class="km-btn km-btn-primary">⚡ Run Assign Step</button>
      <button type="button" id="km-ec-reset" class="km-btn km-btn-outline">🔄 Reset Scenario</button>
    </div>
  </div>

  <div class="kmeans-explanation" id="km-ec-explanation">
    <div class="exp-block">
      <div class="exp-title">🌱 Scenario Initialized (Extreme Collapse)</div>
      <div class="exp-desc">Centroid 1 is placed inside the data cluster. Centroids 2 & 3 start far out in empty space.</div>
    </div>
    <div class="exp-block">
      <div class="exp-title">⚡ Action</div>
      <div class="exp-desc">Click <strong>Run Assign Step</strong> to calculate Euclidean distance to nearest centroid.</div>
    </div>
  </div>
</div>
...
.empty-cluster-demo .kmeans-canvas-wrapper {
  margin-bottom: 10px;
}
.empty-cluster-demo canvas {
  height: 240px !important;
}
.empty-cluster-demo .kmeans-explanation {
  min-height: 80px;
  margin-top: 10px;
  padding: 10px 14px;
}
...
(function() {
  ...
  const DEEP_SEA_PALETTE = [
    { fill: 'rgba(94, 231, 255, 0.85)', stroke: '#5ee7ff', centroid: '#5ee7ff' },
    { fill: 'rgba(255, 110, 199, 0.85)', stroke: '#ff6ec7', centroid: '#ff6ec7' },
    { fill: 'rgba(255, 217, 102, 0.85)', stroke: '#ffd966', centroid: '#ffd966' }
  ];
  const DARK_PALETTE = [
    { fill: 'rgba(0, 243, 255, 0.8)', stroke: '#00f3ff', centroid: '#00f3ff' },
    { fill: 'rgba(255, 78, 80, 0.8)', stroke: '#ff4e50', centroid: '#ff4e50' },
    { fill: 'rgba(255, 204, 0, 0.8)', stroke: '#ffcc00', centroid: '#ffcc00' }
  ];

  function getThemeConfig() {
    if (document.body.classList.contains('light-theme')) {
      return { palette: LIGHT_PALETTE, unassigned: 'rgba(140, 120, 105, 0.45)', grid: 'rgba(61, 46, 37, 0.08)', centroidSymbol: '#3d2e25', connectingAlpha: 0.2 };
    }
    if (document.body.classList.contains('sky-theme')) {
      return { palette: SKY_PALETTE, unassigned: 'rgba(100, 130, 160, 0.45)', grid: 'rgba(79, 157, 232, 0.08)', centroidSymbol: '#1e3a5f', connectingAlpha: 0.2 };
    }
    if (document.body.classList.contains('garden-theme')) {
      return { palette: GARDEN_PALETTE, unassigned: 'rgba(154, 223, 181, 0.35)', grid: 'rgba(154, 223, 181, 0.08)', centroidSymbol: '#ffffff', connectingAlpha: 0.2 };
    }
    if (document.body.classList.contains('deep-sea-theme') || document.body.classList.contains('galaxy-night-theme')) {
      return { palette: DEEP_SEA_PALETTE, unassigned: 'rgba(94, 231, 255, 0.35)', grid: 'rgba(94, 231, 255, 0.06)', centroidSymbol: '#ffffff', connectingAlpha: 0.18 };
    }
    if (document.body.classList.contains('galaxy-theme')) {
      return { palette: GALAXY_PALETTE, unassigned: 'rgba(167, 139, 250, 0.35)', grid: 'rgba(167, 139, 250, 0.06)', centroidSymbol: '#ffffff', connectingAlpha: 0.18 };
    }
    return { palette: DARK_PALETTE, unassigned: 'rgba(160, 175, 200, 0.4)', grid: 'rgba(255, 255, 255, 0.04)', centroidSymbol: '#ffffff', connectingAlpha: 0.12 };
  }

  let canvas, ctx;
  let rawPoints = []; // relative coordinates [0..1]
  let centroids = []; // relative coordinates [0..1]
  let scenario = 'collapse';
  let stepState = 0;

  function initDemo() {
    canvas = document.getElementById('km-ec-canvas');
    if (!canvas) return;
    ctx = canvas.getContext('2d');

    setupScenario('collapse');
    bindEvents();
  }
  ...
  function setupScenario(mode) {
    ...
    for (let i = 0; i < 95; i++) {
      ...
      const z1 = Math.sqrt(-2.0 * Math.log(u1)) * Math.sin(2.0 * Math.PI * u2);
      rawPoints.push({
        rx: Math.min(0.82, Math.max(0.08, centerX + z0 * 0.09)),
        ry: Math.min(0.85, Math.max(0.12, centerY + z1 * 0.14)),
        cluster: -1
      });
    }
  ...
  function runStep() {
    ...
    if (stepState === 0) {
      rawPoints.forEach(p => {
        const px = p.rx * width, py = p.ry * height;
        let minDist = Infinity;
        let best = -1;
        centroids.forEach((c, idx) => {
          const cx = c.rx * width, cy = c.ry * height;
      ...
      stepState = 1;
    } else if (stepState === 1 && scenario === 'recover') {
      const counts = [0, 0, 0];
      rawPoints.forEach(p => counts[p.cluster]++);
      ...
      centroids.forEach((c, idx) => {
        if (counts[idx] === 0) {
          ...
          rawPoints.forEach(p => {
            const px = p.rx * width, py = p.ry * height;
            const parentC = centroids[p.cluster];
            const cx = parentC.rx * width, cy = parentC.ry * height;
            const d = Math.hypot(px - cx, py - cy);
            if (d > maxDist) {
              maxDist = d;
              maxPt = p;
  ...
  function updateUI() {
    const activeEl = document.getElementById('km-ec-active');
    const statusEl = document.getElementById('km-ec-status');
    const expEl = document.getElementById('km-ec-explanation');

    const counts = [0, 0, 0];
    rawPoints.forEach(p => { if (p.cluster >= 0) counts[p.cluster]++; });
    const activeCount = counts.filter(c => c > 0).length;
    const emptyCount = 3 - activeCount;

    if (activeEl) activeEl.innerText = `${activeCount} / 3`;

    if (stepState === 0) {
      if (statusEl) statusEl.innerText = 'Initialized';
      if (expEl) expEl.innerHTML = `
        <div class="exp-block">
          <div class="exp-title">🌱 Scenario Initialized (${scenario === 'collapse' ? 'Extreme Collapse' : 'Scikit-Learn Recovery'})</div>
          <div class="exp-desc">Centroid 1 is placed inside the data cluster. Centroids 2 & 3 start far out in empty space.</div>
        </div>
        <div class="exp-block">
          <div class="exp-title">⚡ Action</div>
      ...
      if (emptyCount > 0) {
        if (statusEl) statusEl.innerText = `⚠️ ${emptyCount} Empty Cluster${emptyCount > 1 ? 's' : ''}`;
      } else {
        if (statusEl) statusEl.innerText = '✅ All Clusters Active';
      }

      if (scenario === 'collapse') {
        if (expEl) expEl.innerHTML = `
          <div class="exp-block">
            <div class="exp-title">💥 Extreme Collapse Occurred (<em>K</em> = 1 Capture)</div>
            <div class="exp-desc">Centroid 1 is closest to <strong>100% of data points (${rawPoints.length}/${rawPoints.length})</strong>.</div>
          </div>
          <div class="exp-block">
            <div class="exp-title">⚠️ Mathematical Vulnerability</div>
            <div class="exp-desc">Centroids 2 & 3 received <strong>0 points</strong> (|<em>C</em><sub>2</sub>| = 0, |<em>C</em><sub>3</sub>| = 0). Updating mean would attempt division by zero (0/0)!</div>
          </div>
        `;
      } else {
        if (expEl) expEl.innerHTML = `
          <div class="exp-block">
            <div class="exp-title">⚠️ 2 Empty Clusters Detected (|<em>C</em><sub>k</sub>| = 0)</div>
            <div class="exp-desc">Centroid 1 captured all points. Centroids 2 & 3 are currently empty.</div>
          </div>
          <div class="exp-block">
            <div class="exp-title">⚡ Next Step</div>
            <div class="exp-desc">Click <strong>Run Assign Step</strong> again to trigger Scikit-Learn's <em>Farthest-Point Re-assignment</em> (<em>x</em><sub>max</sub>)!</div>
          </div>
        `;
      }
    } else if (stepState === 2) {
      if (statusEl) statusEl.innerText = '🛠️ Recovered via x_max!';
      if (expEl) expEl.innerHTML = `
        <div class="exp-block">
          <div class="exp-title">🛠️ Scikit-Learn Recovery Executed</div>
          <div class="exp-desc">Detected empty centroids (|<em>C</em><sub>k</sub>| = 0) and forcibly relocated them onto the data points contributing maximum squared error (<em>x</em><sub>max</sub>).</div>
        </div>
        <div class="exp-block">
          <div class="exp-title">🎉 Result</div>
          <div class="exp-desc">All ${activeCount} centroids are now actively clustering data points without crashing or collapsing!</div>
        </div>
      `;
    }
  }
  ...
  function draw() {
    ...
    const height = canvas.clientHeight || canvas.offsetHeight || 240;
    const dpr = window.devicePixelRatio || 1;
    ...
    centroids.forEach((c, idx) => {
      const cx = c.rx * width, cy = c.ry * height;
      const color = theme.palette[idx % theme.palette.length];
      const isEmpty = stepState > 0 && counts[idx] === 0;

      if (isEmpty) {
        ctx.beginPath();
        ctx.arc(cx, cy, 22, 0, Math.PI * 2);
        ctx.fillStyle = 'rgba(255, 78, 80, 0.15)';
        ctx.fill();
        ...
        ctx.fillStyle = color.fill.replace(/0\.\d+/, '0.2');
        ctx.fill();
        ctx.strokeStyle = color.centroid || color.stroke;
        ctx.lineWidth = 2;
        ctx.stroke();
      }

      ctx.strokeStyle = isEmpty ? '#ff4e50' : theme.centroidSymbol;
      ctx.lineWidth = 2.5;
      ctx.beginPath();
      ...
      if (isEmpty) {
        ...
        ctx.fillText('0 points', cx - 20, cy + 32);
      }
    });
  ...
  function bindEvents() {
    const btnStep = document.getElementById('km-ec-step');
    const btnReset = document.getElementById('km-ec-reset');
    const pills = document.querySelectorAll('#km-ec-pills .km-pill-btn');

    if (btnStep) btnStep.addEventListener('click', runStep);
    if (btnReset) btnReset.addEventListener('click', () => setupScenario(scenario));

    pills.forEach(p => {
      p.addEventListener('click', () => {
        pills.forEach(pill => pill.classList.remove('active'));
        p.classList.add('active');
        const mode = p.getAttribute('data-scenario');
        setupScenario(mode);
      });
    });
