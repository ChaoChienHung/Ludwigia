...
透過這種方式，我們就能輕易地將原本需要處理的龐大實體數，利用「不斷分類並在此基礎上繼續拆分」的原理，簡化為單純的「處理分類數」問題。這本質上就是利用數學的**乘法原理（Combinatorial Power）**，大大壓縮了系統需要處理的資訊量。

舉個簡單的例子：假設我們原本需要處理的實體數量高達 1,000,000 個，這是一個極度龐大的數字！但如果我們想辦法將它處理成「100 個大分類 $\times$ 每個大分類裡有 100 個中分類 $\times$ 每個中分類裡有 100 個小分類」，這樣一來，我們真正需要認識與處理的分類數，就只有 100 + 100 + 100 = 300 個了！非常神奇且高效。

因此，人類從不為每個物件單獨編造無意義的隨機代碼，而是習慣將資訊先進行分類，並做「結構化」與「階層化」的梳理。**這，就是 Semantic ID 的底層邏輯！**

<callout>
💡 **為什麼當實體數量一大，就必然能找到可分類的「共通點」？**

要能有效利用乘法原理，關鍵在於這些實體一定要擁有「共通點」，否則這種依賴分類的系統就會失效。但為什麼每當數量一大，我們總能從中找到共通點呢？

這其實與 **大數法則（Law of Large Numbers）** 及高維空間的幾何特性有關。當物品由高維度的特徵描述時，兩個隨機實體要維持完全正交（完全沒有相似點）的機率非常低；如果實體數量極其龐大，要讓所有實體之間完全沒有相關性，這機率幾乎趨近於零。

因此，只要實體數量夠大且維度不低，在純隨機的情況下都必然會存在共通點。更何況真實世界並非隨機：真實世界的商品與內容，往往是出自於相似的人類動機與消費需求，這使得候選商品之間更容易存在高度的相通性。
</callout>
...
由於 Semantic ID 的每個 Token 都代表一個分類語意，因此，在大方向上擁有類似性質的商品，往往可以共享前面的語意 Token，也就是共同「前綴」。
