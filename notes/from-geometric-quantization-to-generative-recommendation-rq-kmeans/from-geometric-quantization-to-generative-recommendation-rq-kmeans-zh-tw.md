...
本文將與讀者一同走過這段從傳統量化到生成式推薦的演進之路，從 K-means 到乘積量化 (PQ)，最終深入探討目前業界主流、CP 值最高且最穩健的終極方案——**RQ-Kmeans (Residual Quantization K-means)**。它能精準地把高維商品向量壓縮成 3~4 個具備由粗到細語意階層的 SID (Semantic ID) Token，完美扮演了生成式推薦中「翻譯官」的角色。
...
但當我們來到幾百維的商品向量空間時，這種做法會立刻撞上「維度詛咒（Curse of Dimensionality）」。真實世界的高維資料分佈往往極度不均，生硬的網格化不僅會產生海量根本沒有資料的「空桶」，白白浪費儲存資源；更致命的是，它完全無法捕捉資料真實的語意邊界。死板的網格線常常會把語意極為相近的商品硬生生切開，分到不同的桶子裡。並且，網格化最大的盲點在於：網格只是一個人為劃定的幾何方塊，它的幾何中心通常無法反映內部資料的真實樣貌，這讓我們無法賦予這個離散 ID 任何具體的「代表性語意」。
...
在幾何與訊號處理領域，**「在向量空間中，直接用聚落中心點（Centroid）的向量，來近似與代表該群落內的所有資料向量」**，這個將無限連續向量空間映射至有限原型向量集的過程，被精煉為一個專有名詞——**向量量化（Vector Quantization, VQ）**。聚類負責找出群落結構，而量化則進一步將該群落內的所有個體，統一用中心點向量取代。
...
### 突破單層極限：殘差量化 (Residual Quantization) 的階層魔法

雖然 VQ 成功把商品變成了離散 ID，但如果我們直接把單層 VQ 餵給自迴歸大模型，會立刻陷入兩個致命的困境：

1. **無法建構階層語意：** 單層 VQ 只會輸出「一個」平面的離散 ID。這就像是給商品貼上一個單純的群集編號，缺乏大模型所需的那種「由粗到細（Coarse-to-fine）」的階層式樹狀結構與因果上下文，Next-Token Prediction 就無從發揮。
2. **單一碼本的兩難（量化失真 vs. 詞表爆炸）：**
   * **若中心點數量 $K$ 太小（如 8192）：** 雖然限制了詞表大小，但量化誤差（Distortion）極大，成千上萬個細節截然不同的商品被強行擠進同一個碼字 ID 中，模型無法分辨商品差異，推薦將變得極度粗糙。
   * **若中心點數量 $K$ 極大（如 100 萬）：** 為了精確代表商品細節，Codebook 大小呈現指數級爆炸（需要數百萬至數億個碼字），這在記憶體儲存與算力上完全不可行，本質上退回了傳統 Atomic ID 詞表爆炸的老路。

**這說明單一碼本（Single Codebook）無法勝任 Tokenizer 的重任。** 為了解決缺乏階層與單碼本兩難的問題，工程界將量化過程從「單次」改為「多層迭代」，發展出了**殘差量化（Residual Quantization, RQ）**。
