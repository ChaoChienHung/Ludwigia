...
在將原始連續向量轉換為離散 Token 序列的過程中（也就是決定上述每層最近中心點的「量化解碼」階段），我們該如何選擇每一層的中心點？實務上有兩種截然不同的策略：
...
了解了 RQ-Kmeans 巧妙的殘差量化機制後，我們可以發現它不僅僅是個單純的壓縮演算法，更是為生成式推薦量身打造的橋樑。具體而言，它具備以下幾個不可替代的核心優勢：
...
5. **高度的架構彈性 (Flexibility of $M$)：**
    這套範式允許我們根據實際的業務需求自主調整碼本層數 $M$。如果我們希望模型推論速度更快（即生成較少的 Token），只需調降 $M$；反之，若我們希望模型能更精確地刻畫並還原商品特徵，則只需提升 $M$。透過調整 $M$，我們能在「推論效率」與「表示精度」之間靈活切換。

然而，RQ-Kmeans 雖然擁有上述諸多優勢與優美的數學幾何直覺，但在真實的工程落地場景中，它並非完美無缺。當面對極端高維且稀疏的真實數據時，純粹的幾何量化會遭遇一些棘手的退化問題與技術挑戰。這也就是我們接下來要探討的進階議題。
...
在理想狀況下，我們會希望碼本中每個 Codeword 的利用率（Codebook Utilization Rate, CUR）達到最大化。畢竟，碼本的容量是我們預先設定好的寶貴資源（如 $K=8192$），如果只有少數幾個中心點被頻繁使用，而其他中心點閒置，就等於白白浪費了模型的表達能力。

然而，就如同在<content-link canonical="k-means-clustering-around-centers">K-Means 聚類</content-link>中有提到的，大規模高維數據上運行 K-means 常常會出現空群集，或著某些聚類中心幾乎沒有數據點落入，導致碼本容量被嚴重浪費，發生碼本坍塌。我們稱這種現象為「死碼（Dead Codes）」現象。

為此，我們通過**正則化殘差量化（Regularized Residual Quantization, RRQ）** 引入了資訊理論中的率失真理論（Rate-Distortion Theory）與反向水閥（Reverse Water-Filling）原則：
...
而儘管 RQ-VAE 結合 AutoEncoder 能達成特徵與量化的聯合優化，看似更先進，但在工業落地時卻面臨巨大挑戰：它高度依賴不可微的 Straight-Through Estimator (STE) 來估算梯度，導致訓練極度不穩定，調參與算力成本高昂。

相比之下，**RQ-Kmeans 擁有絕對優勢**：除了極致穩定之外，它具備極佳的**資產複用性 (Plug-and-Play)**，可無縫接入系統現有的 DSSM 或 Graph 高品質 Item Embedding，無需重訓上游模型。

**💡 實務混用策略：**
其實在實務工程上，這兩者並不互斥。一個常見的做法是：我們先利用 RQ-Kmeans 跑出初始碼本當作良好的初始化權重（Initialization），接著再將其無縫接入 RQ-VAE 去進行微調優化。如此一來，便能同時兼顧初期的模型穩定性，以及後期的極限效能優化！
</block>

至此，我們算是粗略介紹完 RQ-Kmeans 的運作流程了。但業界究竟是如何將之整合到實際的生成式推薦系統中呢？它又在整個推薦系統中扮演什麼樣的角色呢？

## 落地實踐：生成式推薦的四階段工作流

在實際的生成式推薦系統中，RQ-Kmeans 扮演了串聯離線特徵與線上推論的核心組件。整個工作流可分為四個核心階段：

### 階段一：離線訓練量化器 (Offline Training)
1. **獲取物品向量：** 前面有提到，RQ-Kmeans 本質上還是將商品的 Embedding 離散化成階層式 Token 的一個方式。既然如此，我們還是會需要透過預訓練模型（如 DSSM、GNN 或 BERT）為系統中的所有物品生成高品質的連續嵌入向量 $v$。
2. **訓練 RQ-Kmeans：** 接著，我們就可以將全站物品的向量作為輸入，去**訓練（擬合）**一個 $M$ 層的 RQ-Kmeans 量化器，藉由幾何收斂產出 $M$ 個中心點碼本 $\mathcal{C}^{(1)}, \mathcal{C}^{(2)}, \dots, \mathcal{C}^{(M)}$。

### 階段二：離線商品 Token 化與總詞彙表 (Offline Encoding)
此階段屬於純推論（Inference）的編碼過程，並不會更新碼本權重：
1. **編碼全站物品：** 將每個物品的嵌入向量輸入訓練好的 RQ-Kmeans，利用 Beam Search 等策略生成長度為 $M$ 的 SID 序列。例如：
...
1. **轉換與展平用戶序列：** 在有了階段二生成的總詞彙表後，我們就可以將用戶的歷史交互序列轉換成 Token 序列並展平：
   * 原始序列：$[\text{Item}_A, \text{Item}_B]$
   * Token 化：$[[12, 255, 7, 98], [56, 101, 23, 42]]$
   * 展平輸入模型：$[12, 255, 7, 98, 56, 101, 23, 42, \dots]$
2. **生成式訓練：** 接著，將展平後的 Token 序列送入 Transformer，我們就可以開始真正訓練生成式自回歸模型預測下一個 Token (Next-Token Prediction) 的能力。

### 階段四：線上推論、重構與召回 (Inference)
線上服務時，輸入使用者的最新歷史，模型會自迴歸地生成下一個目標物品的 $M$ 個 Tokens $[\text{idx}_1, \dots, \text{idx}_M]$。
關於接下來的推論召回方式，實務上通常有兩種不同的路徑設計：

* **路徑一（向量重構與 ANNS 召回）：**
  從碼本中取出生成的 Tokens 對應的中心向量並求和，重構出預測的興趣向量 $v_{\text{pred}}$。接著，利用 Faiss 或 Milvus 等 **ANNS（近似最近鄰搜尋, Approximate Nearest Neighbor Search）** 引擎，使用 $v_{\text{pred}}$ 完成 Top-$N$ 的高效商品召回。這能有效處理模型生成出「不存在於現有商品庫中」的全新 Token 組合。
* **路徑二（直接 SID 精準比對）：**
  不經過向量重構，而是直接將生成出來的 SID 序列當作索引，去對應實際的商品資料庫進行精準匹配（Exact Match 或 Prefix Match）。這種方式省去了向量搜尋的開銷，速度極快，但高度依賴生成模型的精準度。
