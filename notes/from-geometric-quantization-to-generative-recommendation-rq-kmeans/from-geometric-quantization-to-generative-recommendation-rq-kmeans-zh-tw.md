...
因此，我們的首要目標，就是先解決第一個難題：**建立一個有語意的離散碼本**。
...
然而，當來到高維度空間時，這種做法會立刻撞上「維度詛咒（Curse of Dimensionality）」。真實世界的高維資料分佈往往極度不均。生硬的網格化不僅會產生海量的「空桶」，白白浪費儲存與運算資源；更致命的是，它完全無法捕捉資料真實的語意邊界——也就是說，死板的網格線常常會把語意相近的資料硬生生切開，無法保證能將它們分在同一個桶子裡。

除此之外，網格化還面臨另一個根本性的痛點：**它無法建構出真正帶有語意的「碼本（Codebook）」與「碼字（Codeword）」**。網格只是一個人為劃定的死板幾何方塊，其幾何中心通常無法反映內部資料的真實樣貌，這讓賦予該區域語意這件事變得毫無根據。

這不禁讓我們思考：與其用一把死板的尺去切割空間，有沒有方法能順應相似語意資料本身的分佈，做出更聰明、有效的劃分？又或者說，我們能不能先用某種演算法，找出一群群具有相似語意的族群，**並用各族群真實的中心點（Centroids）來作為「碼字（Codeword）」，進而建構出我們的「碼本（Codebook）」呢？**
...
<block>
**迷思破解：既然一開始我們不能只使用單一 Token 來表示商品是因為物品數量太多，那我們透過上述 K-means 的方式把它們聚類成 8192 個群，這樣單一 Token 的可能性不就限縮到 8192 了嗎？直接用這 8192 個聚類中心當作單一 Token 的碼本不就好了嗎？**

這個想法很直覺：既然幾億個商品太多，那就把它們聚類成 8192 個群，詞彙表不就縮小了嗎？但如果我們只依賴這種單一碼本的範式，會立刻陷入兩難的困境：
* **若 $K$ 值太小（例如 8192）：** 雖然解決了詞表過大的問題，但量化誤差（Distortion）會非常驚人。這意味著可能會有成千上萬個截然不同的商品被強行映射到同一個 Codebook ID 上，模型根本無法區分這些商品的細微差異，推薦將變得極度粗糙且失去意義。
...
並且，透過這種方式，我們僅需極小的儲存空間（$4 \times 256$ 個向量），就能表達高達 $256^4 \approx 43$ 億種商品組合，大幅提升了模型的表達容量。
...
為了克服 Product Quantization 缺乏階層語意的致命缺點，殘差量化（RQ-Kmeans） 成為了技術演進的關鍵突破。

與 PQ 粗暴切分空間不同，RQ 選擇保留完整的全局向量，改採「逐層細化、遞減殘差、逐步求精」的策略：
...
然而，就如同在<content-link canonical="k-means-clustering-around-centers">K-Means: Clustering Around Centers</content-link>中所探討的，在大規模高維數據上運行 K-means 常常會出現空群集（Empty Clusters），或是某些聚類中心幾乎沒有數據點落入。這會導致碼本有效容量被嚴重稀釋，發生碼本坍塌，在量化領域中被稱為「死碼（Dead Codes）」現象。
...
而儘管 RQ-VAE 透過 AutoEncoder 架構實現特徵學習與量化碼本的聯合優化，理論上限更高；然而在工業落地時，它高度依賴不可微的 Straight-Through Estimator (STE) 來近似梯度，容易面臨碼本坍塌與訓練發散的風險，調參與算力成本顯著偏高。
...
至此，我們總算是粗略介紹完了生成式推薦的背景以及 RQ-Kmeans 在其中扮演的角色與運作流程。
...
RQ-Kmeans 具備「極致穩定、無需梯度傳播、可無縫複用現有資產」的巨大優勢。此外，其模型生成的全新 Token 組合所重構出的向量，更為推薦系統的探索性開啟了前瞻想像空間。在走向生成式檢索的浪潮中，RQ-Kmeans 堪稱工業界連接連續向量空間與離散 LLM 語意序列最為實用、堅固的橋樑。
